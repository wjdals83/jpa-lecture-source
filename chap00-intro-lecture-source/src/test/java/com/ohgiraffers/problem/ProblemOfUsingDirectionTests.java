...
public class ProblemOfUsingDirectionTests {
    ...
    /* 수업목표. 테스트 코드 기반으로 JPA 를 사용하지 않았을 떄 문제를 알아보자 */
    ...
    *  */
    ...
    /* 2-1. 조회 항목 요구사항 변화에 따른 의존성 문/제 */

    /* 필기.
    *   고객 즉 클라이언트 측의 요구사항 변화로 인한
    *   DB 수정, SQL 문 수정, Application 수정 등의 문/제 발생
    *  */
    ...
    /* 3. 패러다임 불일치 문/제 (상속, 다형성, 캡슐화, 추상화) */

    /* 필기.
    *   객체 지향 언어의 상속과 유사한 것이 데이터베이스의 서브타입 엔티티 이다.
    *   유사한 것 같지만 다른 부분은 데이터베이스의 상속은 상속 개념을 데이터로 추상화 해서
    *   슈퍼타입과 서브타입으로 구분하고, 물리적으로는 다른 테이블로 분리가 되니 형태이다.
    *  */

    /* 3-1. 연관관계 문/제 */

    /* 객체지향에서 말하는 1 대 1 관계 1대 다 관계는 데이터베이스 저장 구조와 다른 형태이다. */
